...
cfg_if! {
    if #[cfg(target_arch = "wasm32")] {
        ...
        /// The signaller used when none is set (z2rs patch: exported so a
        /// caller can wrap it, e.g. to observe signalling progress).
        pub type DefaultSignallerBuilder = wasm::WasmSignallerBuilder;
        ...
        /// The signaller used when none is set (z2rs patch: exported so a
        /// caller can wrap it, e.g. to observe signalling progress).
        pub type DefaultSignallerBuilder = native::NativeSignallerBuilder;
        /// A future which runs the message loop for the socket and completes
        /// when the socket closes or disconnects
