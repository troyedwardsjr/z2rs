...
pub struct NativeSignallerBuilder;
...
async fn create_rtc_peer_connection(
    ...
    let config = RTCConfiguration {
        // z2rs patch: an empty URL list means "no ICE servers" (host
        // candidates only), not one server entry with no URLs.
        ice_servers: if ice_server_config.urls.is_empty() {
            Vec::new()
        } else {
            vec![RTCIceServer {
