...
/// The built-in websocket signaller (z2rs patch: public so it can be wrapped).
pub struct WasmSignallerBuilder;
...
impl Messenger for WasmMessenger {
    ...
    async fn offer_handshake(
        ...
        // z2rs patch: bounded, see `ICE_GATHERING_TIMEOUT`.
        let late_candidates = wait_for_ice_gathering_complete(conn.clone()).await;
...
async fn complete_handshake(
    ...
    late_candidates: Option<LateCandidates>,
    mut data_channels_ready_fut: Pin<Box<futures::future::Fuse<impl Future<Output = ()>>>>,
    mut peer_signal_rx: UnboundedReceiver<PeerSignal>,
) {
    let trickle_peer = signal_peer.clone();
    let onicecandidate: Box<dyn FnMut(RtcPeerConnectionIceEvent)> = Box::new(
        move |event: RtcPeerConnectionIceEvent| {
            let candidate_json = candidate_json(&event);
            debug!("sending IceCandidate signal: {candidate_json:?}");
            trickle_peer.send(PeerSignal::IceCandidate(candidate_json));
        },
    );
    let onicecandidate = Closure::wrap(onicecandidate);
    conn.set_onicecandidate(Some(onicecandidate.as_ref().unchecked_ref()));
    // z2rs patch: forward the candidates gathered after the description was
    // sent early. This runs synchronously right after the handler swap, so no
    // candidate event can fall between the buffer and the live handler.
    if let Some(mut late) = late_candidates {
        while let Ok(candidate_json) = late.rx.try_recv() {
            debug!("sending late IceCandidate signal: {candidate_json:?}");
...
fn create_rtc_peer_connection(ice_server_config: &RtcIceServerConfig) -> RtcPeerConnection {
    ...
    // z2rs patch: an empty URL list means "no ICE servers" (host candidates
    // only, gathering completes at once), not one server entry with no URLs.
...
/// z2rs patch: the longest a local description waits for ICE gathering.
///
/// Upstream waits for gathering to *complete*, and a browser only completes
/// once every configured STUN/TURN server has answered or timed out. One
/// unresponsive server (observed with the default Google STUN pair) held the
/// offer back for about 40 s, so two peers on the same machine sat idle that
/// long. Host and server-reflexive candidates normally arrive within tens of
/// milliseconds; anything gathered after this bound is trickled instead (see
/// [`LateCandidates`]), so nothing is lost by sending early.
const ICE_GATHERING_TIMEOUT: Duration = Duration::from_millis(1000);

/// Candidates gathered after a description was sent before gathering
/// completed. Buffered until the trickle handler in `complete_handshake`
/// takes over.
struct LateCandidates {
    rx: UnboundedReceiver<String>,
    _handler: Closure<dyn FnMut(RtcPeerConnectionIceEvent)>,
}

fn candidate_json(event: &RtcPeerConnectionIceEvent) -> String {
...
async fn wait_for_ice_gathering_complete(conn: RtcPeerConnection) -> Option<LateCandidates> {
    ...
    let completed = matches!(
        futures::future::select(rx.next(), Box::pin(Delay::new(ICE_GATHERING_TIMEOUT))).await,
        futures::future::Either::Left(_)
    );

    conn.set_onicegatheringstatechange(None);
    if completed || conn.ice_gathering_state() == RtcIceGatheringState::Complete {
        debug!("Ice gathering completed");
        return None;
    }

    warn!(
        "ice gathering still running after {ICE_GATHERING_TIMEOUT:?}; sending the description now and trickling the rest"
    );
    let (late_tx, late_rx) = futures_channel::mpsc::unbounded();
    let handler: Box<dyn FnMut(RtcPeerConnectionIceEvent)> =
        Box::new(move |event: RtcPeerConnectionIceEvent| {
            let _ = late_tx.unbounded_send(candidate_json(&event));
        });
    let handler = Closure::wrap(handler);
    conn.set_onicecandidate(Some(handler.as_ref().unchecked_ref()));
    Some(LateCandidates {
        rx: late_rx,
        _handler: handler,
    })
